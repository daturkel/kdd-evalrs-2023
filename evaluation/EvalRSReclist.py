...
from reclist.reclist import CHART_TYPE, RecList, rec_test
...
class EvalRSReclist(RecList):
    def __init__(
        ...
        **kwargs
        ...
        super().__init__(model_name, logger, metadata_store, **kwargs)
        self.dataset = dataset
        self._x_train = dataset._get_train_set(fold=0)
        subset_of_cols_to_return = ["user_id", "track_id"]
        self._x_test = dataset._get_test_set(
            fold=0, subset_of_cols_to_return=subset_of_cols_to_return
        )[["user_id"]]
        self._y_test = dataset._get_test_set(
            fold=0, subset_of_cols_to_return=subset_of_cols_to_return
        ).set_index("user_id")
    ...
    def miss_rate_equality_difference(
        self,
        y_preds: pd.DataFrame,
        y_test: pd.DataFrame,
        slice_info: pd.DataFrame,
    ...
    def mred_country(self):
        ...
        user_countries = self.dataset.df_users.loc[
            self._y_test.index, ["country"]
        ].fillna("NaN")
        ...
        return self.miss_rate_equality_difference(
            y_pred_valid, y_test_valid, user_countries, "country"
        )
    ...
    def mred_user_activity(self):
        ...
        user_activity = user_activity.groupby("user_id", as_index=True, sort=False)[
            ["user_track_count"]
        ].sum()
        ...
        return self.miss_rate_equality_difference(
            self._y_preds, self._y_test, user_activity, "bins"
        )
    ...
    def mred_track_popularity(self):
        ...
        track_activity = track_activity.groupby("track_id", as_index=True, sort=False)[
            ["user_track_count"]
        ].sum()
        track_activity = track_activity.loc[track_id]

        track_activity["bin_index"] = np.digitize(
            track_activity.values.reshape(-1), bins
        )
        track_activity["bins"] = bins[track_activity["bin_index"].values - 1]

        return self.miss_rate_equality_difference(
            self._y_preds, self._y_test, track_activity, "bins"
        )
    ...
    def mred_artist_popularity(self):
        ...
        artist_activity = artist_activity.groupby(
            "artist_id", as_index=True, sort=False
        )[["user_track_count"]].sum()
        artist_activity = artist_activity.loc[artist_id]

        artist_activity["bin_index"] = np.digitize(
            artist_activity.values.reshape(-1), bins
        )
        artist_activity["bins"] = bins[artist_activity["bin_index"].values - 1]

        return self.miss_rate_equality_difference(
            self._y_preds, self._y_test, artist_activity, "bins"
        )
    ...
    def mred_gender(self):
        ...
        return self.miss_rate_equality_difference(
            self._y_preds, self._y_test, user_gender, "gender"
        )
    ...
    def being_less_wrong(self):
        ...
        misses = hits == False
        miss_gt_vectors = self.similarity_model[
            self._y_test.loc[misses, "track_id"].values.reshape(-1)
        ]
        # we calculate the score w.r.t to the first prediction
        miss_pred_vectors = self.similarity_model[
            self._y_preds.loc[misses, "0"].values.reshape(-1)
        ]
